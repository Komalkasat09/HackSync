...
class ContentRecommender:
    ...
    def _load_resources(self) -> List[Dict[str, Any]]:
        try:
            # Try to open the file - it could be relative or absolute
            filepath = self.resource_file
            
            # If it's a relative path, also try with os.path.abspath
            if not os.path.exists(filepath):
                filepath = os.path.abspath(self.resource_file)
            
            if not os.path.exists(filepath):
                logger.warning(f"Resource file not found at: {filepath} (tried both relative and absolute paths)")
                return []
            
            with open(filepath, 'r', encoding='utf-8') as f:
                data = json.load(f)
                logger.info(f"Loaded {len(data)} resources from {filepath}")
                return data
        except FileNotFoundError:
            logger.error(f"Resource file not found: {self.resource_file}")
            return []
        except json.JSONDecodeError as e:
            logger.error(f"Failed to parse resource file: {e}")
            return []
